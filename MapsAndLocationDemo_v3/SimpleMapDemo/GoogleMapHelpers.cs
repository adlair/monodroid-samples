...
namespace SimpleMapDemo
{
    public static class GoogleMapHelpers
    {
        ...
        /// </summary>
        /// <remarks>This helper will try to add and initialize a <c>MapFragment</c> to a container
        ...
        /// <param name="onMapReadyCallback">An <c>IOnMapReadyCallback</c> object. If this is null, then the helper will check to see if the <c>activity</c> implements the interface and use that.</param>
        public static MapFragment AddMapFragmentToLayout(this AppCompatActivity activity, int resourceId, string tag = "map_frag", IOnMapReadyCallback onMapReadyCallback=null) 
        {
            ...
            {
                if (activity is IOnMapReadyCallback) 
                {
                    mapFrag.GetMapAsync( (IOnMapReadyCallback) activity);
                }
                else
                {
                    throw new ArgumentException("If the onMapReadyCallback is null, then the activity must implement the interface IOnMapReadyCallback.", nameof(activity));
                }
            }
            else 
            {
        ...
        /// <param name="requestCode">A request code that will be returned
        /// to the activity via the <c>OnRequestPermissionsResult</c> method.</param>
        public static void PerformRuntimePermissionCheckForLocation(this AppCompatActivity activity, int requestCode)
        {
            ...
            if (activity.MustShowPerimssionRationale())
            {
                View layoutForSnackbar = activity.FindViewById(Android.Resource.Id.Content);

                var requestPermissionAction = new Action<View>(delegate (View v)
                {
        ...
        /// </summary>
        /// <returns><c>true</c>, it is necessary to show the permissions rationale
        /// dialog, <c>false</c> otherwise.</returns>
        /// <param name="activity">Activity.</param>
        public static bool MustShowPerimssionRationale(this Activity activity)
        {
            bool showShowRationale = false;
